...
class BinaryClassifier:
    ...
    def basic_logo_validator(self, method: str) -> Dict[str, list]:
        ...
        for train_index, test_index in tqdm(logo.split(self.X, self.y, self.groups)):
            ...
            train_ba_score = self.evaluate(y_train, y_preds_train, self.scoring)
            test_ba_score = self.evaluate(y_test, y_preds, self.scoring)
            test_acc_score = self.evaluate(y_test, y_preds, 'accuracy')

            # Append to the results
            balanced_accs.append(test_ba_score)
            accs.append(test_acc_score)
            test_groups.append(self.groups[test_index][0])

            print(f'Test subject {self.groups[test_index][0]} --- Train BA Score: {train_ba_score} --- Test BA Score: {test_ba_score} --- Test Accuracy Score: {test_acc_score}')
    ...
    def evaluate(self, y_trues, y_preds, scoring):
        acc = None
        if scoring == 'accuracy':
